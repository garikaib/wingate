...
export default function Kitchen() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const res = await fetch('/wp-json/wingate-tools/v1/kitchen/public');
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const hero = rootRef.current.querySelector('.kitchen-hero-copy');
      if (hero) {
        gsap.fromTo(hero, { y: 26, opacity: 0 }, { y: 0, opacity: 1, duration: 0.65, ease: 'power2.out' });
      }
      const cards = rootRef.current.querySelectorAll('.kitchen-menu-card');
      if (cards.length) {
  ...
  const downloadExport = async (menuId, format = 'csv') => {
    try {
      const res = await fetch(`/wp-json/wingate-tools/v1/kitchen/menus/${menuId}/export?format=${encodeURIComponent(format)}`);
      if (!res.ok) return;
      ...
      link.href = url;
      link.download = String(data?.fileName || 'kitchen-menu.csv');
  ...
  if (loading) {
    return <div className="bg-brand-gray min-h-screen flex items-center justify-center text-brand-blue">Loading Kitchen...</div>;
  }
    ...
    <div ref={rootRef} className="bg-brand-gray min-h-screen">
      <section className="relative min-h-[55vh] overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 py-16 md:py-20">
          <div className="kitchen-hero-copy max-w-3xl rounded-2xl border border-white/18 bg-white/[0.05] p-6 md:p-8 backdrop-blur-sm">
            <p className="text-xs uppercase tracking-[0.22em] text-brand-yellow font-bold">Club House</p>
            <h1 className="mt-3 font-cinzel text-white text-5xl md:text-6xl">{settings.heroTitle || 'The Kitchen'}</h1>
          ...
          {menus.map((menu) => {
            ...
            return (
              <button
                ...
                className={`kitchen-menu-card text-left overflow-hidden rounded-xl border ${isActive ? 'border-brand-yellow shadow-lg' : 'border-brand-blue/12 shadow-sm'} bg-white transition-all hover:-translate-y-1`}
              >
      ...
      <section className="bg-brand-blue text-white">
        <div className="container mx-auto px-4 py-10 grid gap-8 md:grid-cols-3">
          <div>
            <p className="text-[11px] uppercase tracking-[0.16em] text-brand-yellow font-bold">{settings.openTimesTitle || 'Opening Times'}</p>
            <ul className="mt-3 space-y-1 text-sm text-white/90">
              {openTimes.map((line, index) => <li key={`${line}-${index}`}>{line}</li>)}
            </ul>
          </div>
          <div>
            <p className="text-[11px] uppercase tracking-[0.16em] text-brand-yellow font-bold">Kitchen Promise</p>
            <p className="mt-3 text-sm text-white/90">Fresh ingredients, polished service, and menu windows designed around your day at the club.</p>
          </div>
          <div className="md:text-right">
            <a href={settings.bookingUrl || '/booking/'} className="inline-flex items-center gap-2 rounded-sm bg-brand-yellow px-6 py-3 text-xs font-bold uppercase tracking-widest text-brand-blue !no-underline hover:bg-white transition-colors">
              ...
              <span>{settings.bookingLabel || 'Book A Table'}</span>
              <ArrowRight size={14} />
            </a>
            {settings.whatsAppEnabled && settings.whatsAppNumber ? (
              <a
                href={`https://wa.me/${String(settings.whatsAppNumber).replace(/\D/g, '')}?text=${encodeURIComponent(settings.whatsAppMessage || 'Hello Wingate Kitchen, I would like to reserve a table.')}`}
                ...
                rel="noreferrer"
              >
                <span>{settings.whatsAppLabel || 'WhatsApp Reservations'}</span>
                <ArrowRight size={14} />
              </a>
            ) : null}
          </div>
